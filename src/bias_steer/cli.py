...
def main(argv=None) -> int:
    ...
    prog = lambda it, **kw: tqdm(list(it), **kw)  # noqa: E731

    # Refusal-direction repro: substring scoring, so NO OpenAI key required.
    if args.command == "refuse":
        print(f"refusal repro: {cfg.label}")
        print(f"  models:  {', '.join(cfg.models)}")
        print(f"  dataset: {cfg.dataset.name}  judge: {cfg.judge.name} (substring; no API key)\n")
        from . import experiment_refusal
        results = experiment_refusal.run_refusal(cfg, runs_dir=args.runs_dir,
