...
| **WP-31** | System-prompt control baseline on `qwen3-8b` (`docs/HANDOFF_prompt_control.md`, steering-hygiene bar) | **§1 done, valid; §2 done twice (contaminated vector, then clean re-extraction) — both show prompt strictly dominating, no complementarity** | Claude | §1: prompt-only baseline passes its sanity gate. §2: per-item steer-vs-prompt comparison (needed-experiments §14) | §1: `runs/20260903-093536_prompt-baseline-opinion_qwen3-8b` — sanity gate PROMPT_POS 200/200 opinionated, PROMPT_NEG 200/200 neutral. §2 (attempt 1, contaminated vector pulled from `fk/qwen3-8b-opinion-vector`): `runs/20260903-102805_prompt-baseline-opinion_qwen3-8b` — opinion Δ=−0.035 [−0.055,−0.015], neutral Δ=−0.260 [−0.310,−0.210], steer-only=0 both directions. §2 (attempt 2, clean re-extraction — `configs/exp/anchor_qwen3_8b.py` with `enable_thinking=False`, run `runs/20260903-105600_anchor-qwen3-8b_qwen3-8b`, applied in `runs/20260903-110706_prompt-baseline-opinion_qwen3-8b`, merged in that dir's `comparison_summary.md`): **opinion Δ=−0.195 [−0.240,−0.150], neutral Δ=−0.605 [−0.660,−0.545], steer-only=0 both directions — WORSE than the contaminated vector, not better.** Fixing the truncation bug did not recover the complementarity a prior (pre-fix, thinking-mode) run of this same experiment found — prompt strictly dominates in both attempts, CIs clear of zero, zero items where steering alone succeeds. Open question, not investigated: whether the weaker neutral-direction steering is because non-thinking INITIAL is 70% opinionated by default (`runs/20260903-105600.../summary.md`), starving the TRAIN split's neutral bucket, vs. coeff=8.0 being uncalibrated for non-thinking-mode's activation geometry, vs. complementarity being a genuinely thinking-mode-specific effect. Also fixed along the way: `save_residuals()` was missing `n_layers`/`d_model` kwargs its own body required (`src/bias_steer/artifacts.py`) — a real, previously-silent crash blocking any qwen3-8b extraction, not just a test artifact (`tests/test_phase2.py` now 9/9). | N |
