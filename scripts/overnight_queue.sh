...
# DOSE GRID: the declared doses are KEPT and one measured-realistic dose is
# ADDED. Decided 2026-09-02 by Jeremiah: every model must be run at the same
# settings or the cross-model comparison is not a comparison, so a grid that
# turns out to be badly chosen gets EXTENDED, not replaced.
#
# What the measurement said (qwen-14b/Age, coherent completions out of 12):
#   0.00 12   0.01 12   0.02 12   0.05 12   0.10 11   0.20 degenerate
#   0.35 0    0.50 0
# At 0.35+ every condition -- plus, minus AND the norm-matched random control
# -- emits one repeated CJK token while the unsteered baseline answers in
