...
The recipe itself kept improving in parallel. Conditional activation steering (arXiv:2409.05907) fires an intervention only when a condition vector detects the relevant context. Affine concept editing (arXiv:2411.09003) treats refusal as an affine function — projection plus addition relative to a reference point — rather than a pure additive direction. Hypernetwork-generated vectors (arXiv:2506.03292) match prompting on the AxBench evaluation. And activation *capping* — clamping a projection at a percentile rather than adding a constant multiple of a vector — has largely displaced fixed coefficients, which is directly relevant to this project given what the audits found about per-layer norms.

There is also now a side-effect literature, which matters because it is where honest negative results have been landing. Steering toward a narrow behavior can induce broad emergent misalignment (arXiv:2606.08682) and silently weaken safety (arXiv:2603.24543). One preregistered study (arXiv:2607.17427) shows that removing the refusal direction shifts unrelated dispositions: models that have had refusal surgically removed become measurably more optimistic and hedge less. That paper explicitly asks somebody to run the dissociation experiment this project has been circling.

Meanwhile the technique went into production. One major lab industrialised precisely this pipeline as persona vectors (arXiv:2507.21509) — automated trait to contrastive prompts to difference-in-means vector — using it for monitoring, for preventative steering during finetuning, and for flagging training data. The same lab shipped a deployed-grade activation-capping result (arXiv:2601.10387). Another ships activation-based probes in production. And a startup deprecated its self-serve steering API in early 2026: monitoring with directions found product-market fit, consumer-facing steering did not.
...
Sparse autoencoders are alive as infrastructure and demoted as ontology: large open latent sets ship, but at least one major lab publicly pivoted from SAE basic science toward probes and model biology after negative downstream results, and there are now results showing SAEs dilute manifolds across redundant latents (arXiv:2604.28119) with geometry-walled scaling behavior (arXiv:2605.09887). The strongest current alternative for steering purposes is a mixture-of-factor-analyzers region method (arXiv:2602.02464) reporting roughly 96 percent interpretable features against roughly 29 percent for SAEs.
...
Political-neutrality steering also exists (arXiv:2508.08846), with a multilingual extension across 50 countries finding that ideology directions do not align across languages by default (arXiv:2601.23001), and a continuous censorship dial (arXiv:2504.17130). The important distinction those papers force is between **opinionation** — whether the model takes *a* side, a scalar — and **ideology** — *which* side, a direction. They steer which; this project steers whether.

The labs professionalised measurement in parallel: an open-sourced paired-prompts even-handedness evaluation whose stated ideal is symmetric engagement rather than refusal, a five-axis political-bias framework carrying "political refusal" as a named axis, and a consistency-training paper that penalises "fence-sitting" by name (arXiv:2605.22771). Benchmarks moved too: IssueBench (arXiv:2502.08395), with 2.49 million prompts across 212 issues, is the standard now, CrowS-Pairs is widely criticised, and reviewers will ask why anyone used BBQ (arXiv:2110.08193) as an opinion resource when it is a bias-in-question-answering benchmark.
