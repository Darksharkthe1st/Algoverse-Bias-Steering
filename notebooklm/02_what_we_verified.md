...
The smallest model in the set goes from 30 opinionated responses at baseline to 75 when steered toward opinion and 2 when steered toward neutrality. The largest goes from 6 to 24 to 12. One model, a 2-billion-parameter checkpoint that was already highly opinionated at baseline with 82, saturates completely at 96 out of 96 when steered toward opinion — not a single neutral response left in the arm. The bidirectional steering effect is large, it is auditable, it is not a spreadsheet artifact, the spreadsheets were not hand-edited, and the per-record evidence survives on disk. A committed script re-proves it and exits with a status code, so anyone can re-run the proof in seconds without a GPU.
...
The first one recounted from the **text logs** — the human-readable transcripts the 2025 pipeline wrote alongside everything else. It built a parser that finds judgement markers as exact full lines, handles two different filename conventions, refuses to guess at ambiguous label spaces, tracks unparsed labels separately rather than silently dropping them, and hashes every source log it reads. Across 392 archived steering text logs it found 61,794 exact judgement markers and reproduced the archived count tables for twelve further rows across two other spreadsheets.

The second one recounted from the **response pickles** — the serialised Python objects holding the raw generations and labels. Different code, different author, different files, different failure modes.

They agree. That agreement is worth more than either audit alone, because the two artifact families could have disagreed in informative ways: a parser bug lives in the text-log route and not the pickle route, a serialisation bug lives in the pickle route and not the text-log route, and a genuine error in the underlying experiment lives in both. When two routes with disjoint bug surfaces produce the same numbers, what remains is either the truth or a fault upstream of both — and upstream of both is exactly where the project went looking next.
...
The first is a shape bug. A steering vector for this method is supposed to be a two-dimensional tensor of shape *number of layers by model width* — one direction per layer. The archived refusal vectors are **one-dimensional tensors of hidden width**: a 2048-element vector for one model, 4096 for another. The steering code does the same thing in both cases — it writes `steering_vector[layer]`. On a correctly-shaped two-dimensional stack, indexing by layer hands you a direction, which is why the opinion arm is sound. On a one-dimensional tensor, indexing by layer hands you **a single number**, which then broadcasts across the entire residual width. What the model received was a uniform DC offset added to every dimension of its internal state. Not a wrong direction. Not a weak direction. Not a direction at all.
...
**The response pickles are cumulative.** The first log holds 96 records. The second holds 192. The seventh holds 672. Each file contains every model run so far, appended, and only the final 96 records belong to the model named in the filename. Loading one of these and counting everything gives a number that belongs to no model and matches no row — and, crucially, looks plausible. Since the first task of the new sprint is to re-judge the archived outputs, this is a live trap sitting directly in front of the next person to touch this data.
